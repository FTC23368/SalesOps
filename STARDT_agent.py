...
class StardtAgent:
    ...
    def generate_response(self, message_history: list[BaseMessage]):
        ...
        stardt_prompt = get_prompt("stardt").format(user_query=user_query)
        llm_messages = create_llm_msg(stardt_prompt, message_history)
        return self.model.stream(llm_messages)
    
    def stardt_agent(self, state: dict) -> dict:
        return {
            "lnode": "stardt_agent",
